...
10. Configure the MySQL backup automatically over verified key-only SSH; do not ask the user to configure it in CloudPanel. Install an adapted `assets/server/mysql-backup.sh.tpl` outside release directories, write the transient database credential only to a mode-`600` client config outside the repository, and install the idempotent cron entry from `assets/server/mysql-backup.cron.tpl`. Run it immediately, verify a nonempty gzip artifact, enforce daily retention from one to five days with five as the default, and perform a restore drill into an isolated target. Use `scripts/verify_backup_artifact.py` when the artifact is filesystem-accessible. Ask for administrator help only when the verified deploy account cannot provide the required native tools, scheduler, writable backup location, or isolated restore target.
