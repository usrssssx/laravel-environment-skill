...
At the deploy-user checkpoint, the skill acts first: choose a proposed deploy username, generate a dedicated ED25519 pair outside the repository, and show only the public key. Ask the user to create that user for the intended site and add the key. Do not ask for a password unless the user says direct key installation is unavailable. Never ask for later-stage database or portal values early.

At the CloudPanel MySQL checkpoint, first show the user-provided instruction link and ask the user to create a database for the test site. Then ask exactly for the database name, database username, and database password shown by CloudPanel. Set `database.management=cloudpanel`, `database.host=127.0.0.1`, and `database.port=3306` internally. Never require the user to know those inferred values, and never save or repeat the password in JSON, Markdown, Git, reports, or later chat messages. An explicitly supplied external MySQL host overrides this default.
