...
At the server-local PostgreSQL checkpoint, ask exactly for the database name, database username, and database password. Set `database.management=native_explicit`, `database.host=127.0.0.1`, and `database.port=5432` internally. Never require the user to know those inferred values, and never save the password in JSON, Markdown, Git, or reports. An explicitly supplied external database host overrides this default.
