...
- Preserve compatible PHP, Composer, Node.js, MySQL/MariaDB, Redis, and Nginx installations. Do not perform major upgrades merely to match a template.
...
- Install PHP extensions required by Laravel and the selected profile. Common extensions include bcmath, ctype, curl, dom/xml, fileinfo, intl, mbstring, openssl, pcntl, tokenizer, and zip; MySQL additionally requires PDO MySQL.
...
- Bind MySQL and Redis to loopback unless remote access is explicitly required and secured.
...
- Install missing PHP extensions and Redis only after checking compatibility with the panel-managed runtime. Reuse CloudPanel's MySQL/MariaDB service instead of installing a competing database service.
...
- Check MySQL with `mysqladmin ping` or a PDO connection and a rolled-back Laravel read/write transaction when selected.
